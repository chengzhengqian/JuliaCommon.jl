# we put the diff code here, as it should be used a general tools
# we add an option for allow only computing the first order
# some auxillary function like prodxxx should be moved to JuliaCommon.jl
module JuliaDiff
using LinearAlgebra
using TensorOperations
using Zygote
export input, input1, @track, callFunc2, ∇, prod322, prod32, prod32_mid, prod23,  prod21, prod31, prod22, is_zero
...
macro track(expr)
    esc(rewrite_call(expr,:(JuliaDiff.callFunc2)))
end
...
function Base.:/(a::Diff2{T},b::Diff2{T}) where T <: AbstractFloat
    @track vector_divide_number(a,b)
end
